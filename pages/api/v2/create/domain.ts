...
import { db } from "/config/db"; // Ensure your db instance is configured correctly
...
export default async function handler(
  ...
  if (req.method === "POST") {
    try {
      ...
      const { email_list, company_enrichment, meta } = response;

      // Validate company enrichment
      if (!company_enrichment && !meta) {
        return res.status(400).json({ error: "Meta and company enrichment data are required" });
      }
      ...
      let companyEnrichmentRecord = null;

      // Conditionally create company enrichment only if it exists
      if (company_enrichment) {
        companyEnrichmentRecord = await db.companyEnrichment.create({
          data: {
            ...
            location: company_enrichment.location
              ? {
                  create: {
      ...
      const uniqueEmailList = Array.from(
        new Set(email_list.map((email) => email.email))
