...
export function formatDomainUrl(url: string) {
  ...
  export async  function trackUsage(type,userId, count = 1 ,dataFor="individual") {
    const usage = {};
    ...
    try {
     await axios.post('http://localhost:3000/api/v2/create/trackUsage', payload);
    } catch (error) {
